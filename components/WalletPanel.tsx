  ...
  if (status === 'connected') {
    return (
      <div className="rounded-lg border border-emerald-700 bg-emerald-950/30 p-3">
        ...
        <div className="mt-1 font-mono text-xs text-zinc-400 truncate">{address}</div>
      </div>
    );
  }
  return (
    <div>
      <button onClick={async () => {
        try {
          ...
          if (nextAddress) onConnect?.(nextAddress);
        } catch {}
      }}
        ...
        {status === 'connecting' ? 'Connecting...' : status === 'detecting' ? 'Detecting wallet...' : 'Connect 1AM Wallet'}
      </button>
